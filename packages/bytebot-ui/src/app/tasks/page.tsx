        ...
        return (
          <Link key={task.id} href={`/tasks/${task.id}`} className="w-full max-w-2xl">
            <motion.div
              ...
              className="group relative overflow-hidden rounded-[28px] border border-white/25 bg-white/10 px-5 py-4 backdrop-blur-2xl shadow-[0_20px_50px_rgba(0,0,0,0.35)]"
            >
              <div
  ...
  const renderGlassBento = () => (
    <div className="grid w-full gap-4 sm:grid-cols-2">
      {Object.entries(groupedByDate).map(([date, list]) => (
        <div
          key={date}
          className="rounded-[24px] border border-white/20 bg-white/10 p-4 backdrop-blur-2xl shadow-[0_20px_50px_rgba(0,0,0,0.35)]"
        >
          <div className="mb-3 flex items-center justify-between text-sm text-white/80">
            <span>{date}</span>
            <span className="rounded-full bg-white/15 px-2 py-1 text-xs text-white/80">
              {list.length} tasks
            </span>
          </div>
            ...
            {list.map((task, idx) => {
              ...
              return (
                <Link key={task.id} href={`/tasks/${task.id}`}>
                  <div className="group relative overflow-hidden rounded-[18px] border border-white/20 bg-white/12 px-3 py-3 shadow-inner shadow-white/10">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      {isFailed ? (
                        <motion.button
                          ...
                          className="rounded-full bg-white/80 px-3 py-1 text-xs font-semibold text-[#0f2d3c] shadow-lg"
                        >
                          ↻ Replay
                        </motion.button>
                      ) : (
          ...
          return (
            <div key={task.id} className={`flex ${isLeft ? "justify-start" : "justify-end"}`}>
              <Link href={`/tasks/${task.id}`}>
                <motion.div
                  whileHover={{ scale: 1.02 }}
                  className="relative w-[480px] max-w-full rounded-[22px] border border-white/20 bg-white/12 p-4 backdrop-blur-2xl shadow-[0_20px_50px_rgba(0,0,0,0.35)]"
                >
                  ...
                  <div className="mb-2 flex items-center justify-between text-xs text-white/70">
                    ...
                    <span className="rounded-full border border-white/25 bg-white/10 px-2 py-1 text-[11px] text-white/80">
  ...
  return (
    ...
    >
      <div className="drag-region relative z-10 w-full max-w-[1200px] rounded-[36px] border border-white/20 bg-white/12 p-6 backdrop-blur-3xl shadow-[0_25px_80px_rgba(0,0,0,0.45)]">
          ...
          <div className="flex flex-1 flex-wrap items-center gap-2">
            {(["ALL", "ACTIVE", "COMPLETED"] as const).map((tab) => (
              ...
              >
                {tab}
                {taskCounts[tab] > 0 && (
                  <span className="ml-2 rounded-full bg-white/15 px-2 py-0.5 text-[10px] text-white/80">
                    {taskCounts[tab]}
                  </span>
                )}
              </button>
          ...
          <div className="flex gap-2">
            {(["floating", "bento", "pulse"] as ViewMode[]).map((mode) => (
              <button
                ...
                className={`no-drag rounded-full px-4 py-2 text-xs font-semibold capitalize transition-all ${
                  viewMode === mode
                    ? "bg-white text-[#0f2d3c] shadow-lg shadow-white/50"
                    : "text-white/80 hover:bg-white/15"
                }`}
        ...
        <div className="rounded-[30px] border border-white/15 bg-white/10 p-6 backdrop-blur-2xl shadow-inner shadow-white/10">
          {isLoading ? (
            ...
            <div className="text-center text-white/70">
              ...
              <Link href="/" className="inline-block rounded-full bg-white/80 px-6 py-3 text-sm font-semibold text-[#0f2d3c] shadow-lg shadow-white/40">
                + Create New Task
              </Link>
            </div>
          ) : (
